...
//package de.hondari.carl.azubi.util;
...
public class Matrix {
    ...
    public void setTwoDarray(double[][] aarray) {
        ...
        for (int i = 0; i < nrow; i++) {
            ...
            System.arraycopy(aarray[i], 0, this.matrix[i], 0, ncol);
        }
    }
     ...
     */
    public void setRow( double[] aarray, int row) {
        if (this.ncol != aarray.length)
            throw new IllegalArgumentException("column length of this Matrix differs from that of the 2D array argument");
        System.arraycopy( aarray, 0, this.matrix[row], 0, this.ncol );
    }
    ...
    public void setSubMatrix(int i, int j, double[][] subMatrix) {
        ...
        int n = k - i + 1, m = l - j + 1;
        for (int p = 0; p < n; p++) {
            System.arraycopy(subMatrix[p], 0, this.matrix[i + p], j, m);
        }
    }
    ...
    public double[][] getArrayCopy() {
        ...
        for (int i = 0; i < nrow; i++) {
            System.arraycopy(this.matrix[i], 0, c[i], 0, ncol);
        }
        return c;
    }
    ...
    public Matrix getSubMatrix(int i, int j, int k, int l) {
        ...
        for (int p = 0; p < n; p++) {
            System.arraycopy(this.matrix[i + p], j, sarray[p], 0, m);
        }
        return subMatrix;
    }
    ...
    public int[] getIndexCopy() {
        ...
        System.arraycopy(this.index, 0, indcopy, 0, this.nrow);
        return indcopy;
    }
    ...
    public static Matrix copy(Matrix a) {
        if (a == null) {
            ...
            System.arraycopy(a.index, 0, b.index, 0, nr);
            return b;
        }
    }
    ...
    public Matrix copy() {
        if (this == null) {
            ...
            System.arraycopy(this.index, 0, b.index, 0, nr);
            return b;
        }
    }
    ...
    public Object clone() throws CloneNotSupportedException {
        Matrix cloneMat = (Matrix) super.clone();
        ...
        for (int i = 0; i < nr; i++) {
            System.arraycopy(this.matrix[i], 0, cloneMat.getArrayReference()[i], 0, nc);
        }
        System.arraycopy(this.index, 0, cloneMat.index, 0, nr);
        return cloneMat;
    }
    ...
    //
    public void timesColumnWiseEquals( double[] columnVector ) {
        if (this.nrow != columnVector.length )
            throw new IllegalArgumentException("Dimension of column vector non-conformant with number of rows in matrix");

        for (int col = 0; col < this.ncol; col++)
            for (int row = 0; row < this.nrow; row++)
                this.matrix[row][col] *= columnVector[row];
    }
    //
    public void timesRowWiseEquals( double[] rowVector) {
        if (this.ncol != rowVector.length )
            throw new IllegalArgumentException("Dimension of column vector non-conformant with number of rows in matrix");

        for (int row = 0; row < this.nrow; row++)
            for (int col = 0; col < this.ncol; col++)
                this.matrix[row][col] *= rowVector[col];
    }


    public double[] sumAllRows() {
        double[] output = new double[ this.ncol ];
        for (int row = 0; row < this.nrow; row++)
            for (int col = 0; col < this.ncol; col++)
                output[col] += this.matrix[row][col];
        return output;
    }
    ...
    public void fliplr() {
        ...
        for (int row = 0; row < this.getNrow(); row++) {
            for (int col = 0; col < ( (double) ncol ) / 2.0; col++) {
                double tmp = getElement( row, col);
                setElement( row, col, getElement( row, ncol - 1 - col));
                setElement( row, ncol-1 - col, tmp);
            }
